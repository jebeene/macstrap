...
show_log_tail() {
  if [[ -f $MACSTRAP_INSTALL_LOG_FILE ]]; then
    local log_lines=$(($TERM_HEIGHT - $LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$MACSTRAP_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Macstrap installation stopped!"
  show_log_tail
  ...
  while true; do
    ...
    if [[ -n ${MACSTRAP_ONLINE_INSTALL:-} ]]; then
      options+=("Retry installation")
    fi

    # Add upload option if internet is available
    # Note: macOS ping uses -W in milliseconds, Linux uses seconds
    ping -c 1 -W 1 1.1.1.1 >/dev/null 2>&1 && options+=("Upload log for support")
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/macstrap/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$MACSTRAP_INSTALL_LOG_FILE"
      else
        tail "$MACSTRAP_INSTALL_LOG_FILE"
      fi
      ;;
    "Exit" | "")
      exit 1
